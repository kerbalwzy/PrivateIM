...
func InsertOneNewSubscription(name, intro, avatar, qrCode string, managerId int64) (*TableSubscription, error) {
	id := SnowFlakeNode.Generate().Int64()
	err := execSqlWithTransaction(InsertOneNewSubscriptionSQL, id, name, managerId, intro, avatar, qrCode)
	if nil != err {
		return nil, err
	}
...
func SelectOneSubscriptionById(id int64, isDelete bool) (*TableSubscription, error) {
	...
	err := row.Scan(&(temp.Id), &(temp.Name), &(temp.ManagerId), &(temp.Intro),
		&(temp.Avatar), &(temp.QrCode), &(temp.IsDelete))
	if sql.ErrNoRows == err {
...
func SelectManySubscriptionByManagerId(managerId int64, isDelete bool) ([]*TableSubscription, error) {
	...
	for rows.Next() {
		temp := new(TableSubscription)
		err := rows.Scan(&(temp.Id), &(temp.Name), &(temp.ManagerId), &(temp.Intro),
			&(temp.Avatar), &(temp.QrCode), &(temp.IsDelete))
		if nil != err {
			continue
		}
...
func UpdateOneSubscriptionIntroById(newIntro string, id int64) error {
	return execSqlWithTransaction(UpdateOneSubscriptionIntroduceByIdSQL, newIntro, id)
}
